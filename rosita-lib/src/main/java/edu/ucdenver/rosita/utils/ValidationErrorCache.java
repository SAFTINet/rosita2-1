...
import edu.ucdenver.rosita.etl.dao.ValidationErrorBatchPreparedStatementSetter;
...
public class ValidationErrorCache {
	...
	public ValidationErrorCache(DataSource ds, Integer threshold, Long dataSourceId) {
		this.dataSource = ds;
		if (dataSource != null) {
		...
		this.validationErrors = new ValidationErrorBatchPreparedStatementSetter(threshold, dataSourceId);
	...
	public void saveAll() {
		if (dataSource != null) {
			jdbc.batchUpdate(sqlTemplates.get("ValidationError"), validationErrors);
			RositaLogger.log(false, "Saved all remaining validation errors");
		}
	}
}
